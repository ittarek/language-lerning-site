...
const Newsletter = () => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // <-- নতুন state

  const handleSubmit = e => {
    e.preventDefault();
    if (!email) return;

    setIsSubmitting(true); // spinner শুরু
    console.log('Newsletter signup:', email);

    // simulate API call
    setTimeout(() => {
      setIsSubscribed(true); // submit success
      setIsSubmitting(false); // spinner বন্ধ
      setTimeout(() => {
        setIsSubscribed(false); // 3 সেকেন্ড পর reset
        setEmail('');
      }, 3000);
            ...
            <form onSubmit={handleSubmit} className="max-w-xl mx-auto">
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                ...
                <SubmitButton
                  ...
                  isSubscribed={isSubscribed} // submit success
                  disabled={false}
                  width={false}
